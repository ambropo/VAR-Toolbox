\documentclass[10pt,english,t,aspectratio=169,ignorenonframetext]{beamer}
\usepackage{etex}
\usepackage{amssymb}
\usepackage{amsfonts}
\usepackage{amsmath}
...
\draw[arrow,opacity=1] (box2.east) to[bend right] +(+0.9,+0.1) node[gold,anchor=north west,opacity=1] {\footnotesize{\textbf{\augiefamily Geometric series}}};
\end{tikzpicture}\pause
...
\item How to know whether is {\small \color{title}[1]} or {\small\color{title}[2]}? This is the {\tikz[tstyle]{\node[nstyle](node1){very nature of the {{\color{note} }}\textbf{identification problem}};}}!
\begin{tikzpicture}[tpstyle]
\draw[pencil,decoration={segment length=1.5pt}] ([yshift=-1pt]node1.south west) to ([yshift=-1pt]node1.south east);
\end{tikzpicture}
\end{itemize}
...
\begin{frame}
\vspace{3cm}\color{title} \centering{\huge \textbf{Common Identification Schemes}}
\end{frame}
...
\begin{frame}
{\textbf{Monetary policy shocks, inflation and unemployment}}\vspace{-.2cm}
...
\begin{frame}
{\textbf{Monetary policy shock:\ The sign restrictions}}\vspace{-.1cm}
...
\hspace{10mm}-1,0,0,0,0,0;  \textcolor{matlabgreen}{\% Deflator }\\
\hspace{10mm}-1,0,0,0,0,0;  \textcolor{matlabgreen}{\% Commodity Price }\\
\hspace{12mm}0,0,0,0,0,0;  \textcolor{matlabgreen}{\% Total Reserves }\\
\hspace{10mm}-1,0,0,0,0,0;  \textcolor{matlabgreen}{\% NonBorr. Reserves }\\
\hspace{12mm}1,0,0,0,0,0]; \textcolor{matlabgreen}{\% Fed Fund }\\
