function [INF,SUP,MED,BAR,B,B_ALL,INF68,SUP68] = VARirband_COVID(VAR,VARopt,SIGN,sr_hor,sr_mod,sr_rot)
% =======================================================================
...
B_ALL = nan(nvar,nvar,ndraws*sr_rot);
...
while tt<=ndraws
    ...
    if tt==1*ww
        disp(' ')
        disp(['Aggregate VAR bootstrap draw: ' num2str(tt)])
        disp('---------------------------------------------------')
        ww=ww+1;
    end
...
%% STEP 4: calculate "ndraws" impulse responses and store them
    ...
    if VAR_draw.maxEig<.9999
        ...
        % Sign restrictions
        VARopt_draw.sr_rot = sr_rot;
        VARopt_draw.sr_hor = sr_hor;
        VARopt_draw.sr_mod = sr_mod;

        % Sign restrictions routine
        [AGGsr, flag] = SR_COVID(VAR_draw,SIGN,VARopt_draw);
        
        % If B matrix is found, save B and IR move counter forward
        if ~flag
            IR(:,:,:,tt) = AGGsr.IR;
            B(:,:,tt) = AGGsr.B;
            B_ALL(:,:,(tt-1)*sr_rot+1:(tt-1)*sr_rot+sr_rot)=AGGsr.Ball;
            tt = tt+1;
        end
...
MED(:,:,:) = prctile(IR(:,:,:,:),50,4);
BAR(:,:,:) = nanmean(IR(:,:,:,:),4);
INF68(:,:,:) = prctile(IR(:,:,:,:),16,4);
SUP68(:,:,:) = prctile(IR(:,:,:,:),84,4);
